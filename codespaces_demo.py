"""
Colt Wayfinder Tool - Codespaces Demo

A completely self-contained demo of the Colt Wayfinder tool optimized for GitHub Codespaces.
"""
...
# Sample data for the demo (same as standalone_demo.py)
SAMPLE_DATA = [
    {
        "id": "p1",
        "type": "product",
...
# HTML template for the demo (same as standalone_demo.py)
...
class ColtWayfinderHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/" or path == "/index.html":
            ...
            self.send_header("Access-Control-Allow-Origin", "*")  # Important for Codespaces
            self.end_headers()
...
def run_server(port=8000):
    """Run the Colt Wayfinder demo server optimized for Codespaces."""
    ...
    with socketserver.TCPServer((host, port), handler) as httpd:
        print(f"Colt Wayfinder demo running at http://{host}:{port}")
        print("In Codespaces, click the 'Open in Browser' button that appears")
